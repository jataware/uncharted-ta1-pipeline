...
def pil_to_cv_image(pil_image: Image.Image, rgb2bgr: bool=False) -> np.ndarray:
    '''
...
def cv_to_pil_image(cv_image: np.ndarray, bgr2rgb: bool=False) -> Image.Image:
    '''
...
def normalize_image_format(im: Image.Image) -> Image.Image:
    '''
    ...
    if im.mode == "F":
        ...
        logger.warning('Raw image is in 32-bit floating point format. Scaling by {} and converting to uint8'.format(pxl_mult))
