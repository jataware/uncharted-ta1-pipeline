...
class MobileNetRCNN(FasterRCNN):
    ...
    def __init__(
        ...
        assert all(i > 0 for i in config["anchor_sizes"]) and isinstance(
            config["anchor_sizes"], Iterable
        )
        assert all(i > 0 for i in config["aspect_ratios"]) and isinstance(
            config["aspect_ratios"], Iterable
        )
        assert config["roi_output_size"] > 0 and isinstance(
            config["roi_output_size"], int
        )
    ...
    def from_pretrained(cls, path: str):
        config = json.load(
            open(os.path.join(path + "/config.json"), "r", encoding="utf-8")
        )
        state_dict = torch.load(
            os.path.join(path + "/state_dict.pt"), map_location=torch.device("cpu")
        )
