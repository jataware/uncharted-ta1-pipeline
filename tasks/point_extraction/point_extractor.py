...
from tasks.point_extraction.legend_item_utils import find_legend_label_matches
...
class YOLOPointDetector(Task):
    ...
    def run(self, task_input: TaskInput) -> TaskResult:
        ...
        """
        image_tiles = ImageTiles.model_validate(task_input.data["map_tiles"])
        ...
        if json_data and image_tiles.join_with_cached_predictions(
            ImageTiles(**json_data), point_legend_mapping
        ):
        ...
        return TaskResult(
            task_id=self._task_id, output={"map_tiles": image_tiles.model_dump()}
        )
