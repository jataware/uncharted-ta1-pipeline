...
class xfund_dataset(Dataset):
    ...
    def load_data(
        ...
        for i in range(len(data_file["documents"])):
            width, height = (
                data_file["documents"][i]["img"]["width"],
                data_file["documents"][i]["img"]["height"],
            )
            ...
            for j in range(len(data_file["documents"][i]["document"])):
                ...
                cur_doc_bboxes.append(
                    self.box_norm(cur_item["box"], width=width, height=height)
                )
        ...
        for i in range(len(total_data["lines"])):
            ...
            for j in range(len(total_data["lines"][i])):
                ...
                assert (
                    len(cur_input_ids)
                    == len([total_data["bboxes"][i][j]] * len(cur_input_ids))
                    == len(cur_labels)
                )
        ...
        for i in range(len(total_input_ids)):
            ...
            while start < len(total_input_ids[i]):
                ...
                input_ids.append(
                    ...
                    + total_input_ids[i][start:end]
                    + [self.tokenizer.sep_token_id]
                )
                bboxs.append(
                    [[0, 0, 0, 0]]
                    + total_bboxs[i][start:end]
                    + [[1000, 1000, 1000, 1000]]
                )
                ...
                image_path.append(
                    os.path.join(
                        self.args.data_dir, "images", total_data["image_path"][i]
                    )
                )
        ...
        data_file = json.load(
            open(
                os.path.join(
                    args.data_dir,
                    "{}.{}.json".format(
                        self.cur_la, "train" if mode == "train" else "val"
                    ),
                ),
                "r",
            )
