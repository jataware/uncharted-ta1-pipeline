...
class Layout_LMv3Embeddings(nn.Module):
    """
...
class Layout_LMv3PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = Layout_LMv3Config
...
class Layout_LMv3SelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class Layout_LMv3Attention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = Layout_LMv3SelfAttention(config)
...
class Layout_LMv3Layer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = Layout_LMv3Attention(config)
        assert (
            not config.is_decoder and not config.add_cross_attention
...
class Layout_LMv3Encoder(nn.Module):
    def __init__(self, config, detection=False, out_features=None):
        super().__init__()
        self.config = config
        self.detection = detection
        self.layer = nn.ModuleList(
            [Layout_LMv3Layer(config) for _ in range(config.num_hidden_layers)]
        )
...
class Layout_LMv3Model(Layout_LMv3PreTrainedModel):
    """ """
    ...
    def __init__(self, config, detection=False, out_features=None, image_only=False):
        ...
        if not self.image_only:
            self.embeddings = Layout_LMv3Embeddings(config)
        self.encoder = Layout_LMv3Encoder(
    ...
    def forward(
        ...
        if images is not None:
            patch_size = 16
            Hp, Wp = int(images.shape[2] / patch_size), int(
...
class Layout_LMv3ClassificationHead(nn.Module):
    """
...
class Layout_LMv3ForTokenClassification(Layout_LMv3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layoutlmv3 = Layout_LMv3Model(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if config.num_labels < 10:
            self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        else:
            self.classifier = Layout_LMv3ClassificationHead(config, pool_feature=False)
...
class Layout_LMv3ForQuestionAnswering(Layout_LMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.qa_outputs = Layout_LMv3ClassificationHead(config, pool_feature=False)
...
class Layout_LMv3ForSequenceClassification(Layout_LMv3PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
