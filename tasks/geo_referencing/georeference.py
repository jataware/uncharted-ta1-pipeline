...
class GeoReference(Task):
    ...
    def run(self, input:TaskInput) -> TaskResult:
        ...
        if num_keypoints < 2 or (abs(max(lon_check) - min(lon_check)) > 40) or (abs(max(lat_check) - min(lat_check)) > 40):
            ...
            geo_projn.estimate_pxl2geo_mapping(list(map(lambda x: x[1], lon_pts.items())), list(map(lambda x: x[1], lat_pts.items())), lon_sign_factor, input.image.size)

        #----- Get lon/lat results for query points for this image
        results = self._process_query_points(query_pts, im_resize_ratio, geo_projn, lon_minmax, lat_minmax, lon_sign_factor)
    ...
    def _process_query_points(self, query_pts: list, im_resize_ratio: float, geo_projn: GeoProjection, lon_minmax: tuple, lat_minmax: tuple, lon_sign_factor: float):

        
        if geo_projn is None:
            return self._add_fallback(query_pts, lon_minmax, lat_minmax, lon_sign_factor)

        # use geographic-projection polynomial to estimate lon/lat for query points
        results = []
        try:
            xy_queries = ([[qp.xy[0]*im_resize_ratio, qp.xy[1]*im_resize_ratio] for qp in query_pts], [[qp.xy[0]*im_resize_ratio + 1, qp.xy[1]*im_resize_ratio] for qp in query_pts], [[qp.xy[0]*im_resize_ratio, qp.xy[1]*im_resize_ratio + 1] for qp in query_pts])
            lonlat_estimates = (geo_projn.predict_xy_pts(xy_queries[0]), geo_projn.predict_xy_pts(xy_queries[1]), geo_projn.predict_xy_pts(xy_queries[2]))

            if lon_minmax and lat_minmax:
                if lon_sign_factor < 0.0:
                    lon_minmax = (lon_sign_factor*max(lon_minmax), lon_sign_factor*min(lon_minmax))

                for lonlat, lonlat_xp, lonlat_yp, qp in zip(lonlat_estimates[0], lonlat_estimates[1], lonlat_estimates[2], query_pts):
                    # ensure query pt results are within expected field-of-view range
                    qp.lonlat = (self._clip(lonlat[0], lon_minmax[0], lon_minmax[1]), self._clip(lonlat[1], lat_minmax[0], lat_minmax[1]))
                    qp.lonlat_xp = (self._clip(lonlat_xp[0], lon_minmax[0], lon_minmax[1]), self._clip(lonlat_xp[1], lat_minmax[0], lat_minmax[1]))
                    qp.lonlat_yp = (self._clip(lonlat_yp[0], lon_minmax[0], lon_minmax[1]), self._clip(lonlat_yp[1], lat_minmax[0], lat_minmax[1]))
                    results.append(qp)
            else:
                for lonlat, lonlat_xp, lonlat_yp, qp in zip(lonlat_estimates[0], lonlat_estimates[1], lonlat_estimates[2], query_pts):
                    qp.lonlat = lonlat
    ...
    def _add_fallback(self, query_pts: list, lon_minmax: tuple, lat_minmax: tuple, lon_sign_factor: float):
        ...
        lon_clue = lon_sign_factor*(lon_minmax[0] + lon_minmax[1])/2    # mid-points of lon/lat clue area
