...
class GeoReference(Task):
    ...
    def run(self, input:TaskInput) -> TaskResult:
        ...
        if num_keypoints < 2 or (abs(max(lon_check) - min(lon_check)) > 40) or (abs(max(lat_check) - min(lat_check)) > 40):
            ...
            geo_projn.estimate_pxl2geo_mapping(list(map(lambda x: x[1], lon_pts.items())), list(map(lambda x: x[1], lat_pts.items())), input.image.size)

        #----- Get lon/lat results for query points for this image
        results = self._process_query_points(query_pts, im_resize_ratio, geo_projn, lon_minmax, lat_minmax, confidence)
    ...
    def _clip_query_pts(self, query_pts:list[QueryPoint], lon_minmax:[float], lat_minmax:list[float]) -> list[QueryPoint]:
        if lon_minmax and lat_minmax:
            for qp in query_pts:
                ...
                qp.lonlat = (self._clip(qp.lonlat[0], lon_minmax[0], lon_minmax[1]), self._clip(qp.lonlat[1], lat_minmax[0], lat_minmax[1]))
                qp.lonlat_xp = (self._clip(qp.lonlat_xp[0], lon_minmax[0], lon_minmax[1]), self._clip(qp.lonlat_xp[1], lat_minmax[0], lat_minmax[1]))
    ...
    def _update_hemispheres(self, query_pts:list[QueryPoint], lon_multiplier:float, lat_multiplier:float) -> list[QueryPoint]:
        for qp in query_pts:
            qp.lonlat = (abs(qp.lonlat[0]) * lon_multiplier, abs(qp.lonlat[1]) * lat_multiplier)
            qp.lonlat_xp = (abs(qp.lonlat_xp[0]) * lon_multiplier, abs(qp.lonlat_xp[1]) * lat_multiplier)
            qp.lonlat_yp = (abs(qp.lonlat_yp[0]) * lon_multiplier, abs(qp.lonlat_yp[1]) * lat_multiplier)
