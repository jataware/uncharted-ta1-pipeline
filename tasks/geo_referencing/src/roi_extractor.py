...
class EntropyROIExtractor(ROIExtractor):
    ...
    def _extract_roi(self, input:TaskInput):
        ...
        if ocr_blocks:
            for blk in ocr_blocks:
                ...
                ocr_pxl_slice = image[ymin:ymax, xmin:xmax, :]  # TODO - or just get median val along the top?
        ...
        entr_img = entropy(image, disk(5))   #image[:,:,0]   # TODO - disk size percentage of image size? (smaller is faster)
        ...
        # with low entropy
        roi_y = self._find_roi_bounds(y_e_avgs, y_vals, entropy_thres, entr_img.shape[0])
        roi_x = self._find_roi_bounds(x_e_avgs, x_vals, entropy_thres, entr_img.shape[1])
        ...
        return [(roi_x[0], roi_y[0]), (roi_x[1], roi_y[0]), (roi_x[1], roi_y[1]), (roi_x[0], roi_y[1])]
    ...
    def _find_roi_bounds(self, avg_vals, p_vals, e_thres, p_len):
        ...
        # then top or left)
        buffer = p_len * 0.01  # TODO could add a buffer here (based on full image dimension or ??)
        roi_p = [0, p_len]
        idx = len(avg_vals) - 1
