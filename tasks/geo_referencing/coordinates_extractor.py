...
class GeoCoordinatesExtractor(CoordinatesExtractor):
    ...
    def _extract_lonlat(
        self, input: CoordinateInput, ocr_text_blocks, lon_minmax, lat_minmax
    ):
        ...
        for idx, (groups, span, _) in dms_matches.items():
            ...
            if abs(deg_decimal - lat_clue) < abs(deg_decimal - lon_clue):
                ...
                if deg_decimal >= lat_minmax[0] and deg_decimal <= lat_minmax[1]:
                    ...
                    coord = Coordinate(
                        "lat keypoint",
                        ocr_text_blocks[idx]["text"],
                        deg_decimal,
                        True,
                        ocr_text_blocks[idx]["bounding_poly"],
                        x_ranges=x_ranges,
                    )
        ...
        if len(coord_lat_results) >= 3 and len(coord_lon_results) >= 3:
            for c in coord_results:
                self._add_param(
                    ...
                    f"coordinate-{c.get_type()}",
                    ocr_to_coordinates(c.get_bounding_box()),
                    f"extracted coordinate - {c.get_text()}",
                )
        ...
        for idx, (groups, span) in deg_matches.items():
            ...
            if idx + 1 in minsec_matches and not is_decimal:
                ...
                font_height = (
                    ocr_text_blocks[idx]["bounding_poly"].vertices[2].y
                    - ocr_text_blocks[idx]["bounding_poly"].vertices[0].y
                )
                blocks_y_separation = (
                    ocr_text_blocks[idx + 1]["bounding_poly"].vertices[0].y
                    - ocr_text_blocks[idx]["bounding_poly"].vertices[2].y
                )
    ...
    def _validate_lonlat_extractions(
        ...
        if num_lon_pts > 4:
            lon_results = self._remove_outlier_pts(
                input, lon_results, im_size[0], im_size[1]
            )
        if num_lat_pts > 4:
            lat_results = self._remove_outlier_pts(
                input, lat_results, im_size[1], im_size[0]
            )
    ...
    def _get_geofence(self, geofence, dms_matches, ocr_text_blocks):
        ...
        for idx, (_, _, deg_parsed) in dms_matches.items():
            ...
            (x, y) = (bounding_box.vertices[0].x + bounding_box.vertices[2].x) / 2, (
                bounding_box.vertices[0].y + bounding_box.vertices[2].y
            ) / 2
            ...
            if abs(utm_dist - northing_clue) < abs(utm_dist - easting_clue):
                ...
                if (
                    ...
                    self._add_param(
                        ...
                        f"coordinate-{coord.get_type()}",
                        ocr_to_coordinates(coord.get_bounding_box()),
                        f"extracted utm coordinate - {coord.get_text()}",
                    )
                else:
    ...
    def _extract_places(
        ...
        for idx, block in enumerate(ocr_text_blocks):
            ...
            if place_dists:
                ...
                x_pixel = self._get_center_x(
                    ocr_text_blocks[idx]["bounding_poly"], (0.0, 1.0)
                )
        ...
        if geo_pts_needed > 0:
            for i in range(geo_pts_needed):
                ...
                coord = Coordinate(
                    "lon keypoint", "", lon_deg, False, pixel_alignment=(x, y)
                )
        ...
        if geo_pts_needed > 0:
            for i in range(geo_pts_needed):
                ...
                y = geo_lonlat_results[i][1][1]
                coord = Coordinate(
                    "lat keypoint", "", lat_deg, True, pixel_alignment=(x, y)
                )
