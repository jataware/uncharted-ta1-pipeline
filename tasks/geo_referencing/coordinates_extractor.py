...
RE_DEGDEC = re.compile(r"^|\b[-+]?([0-9]{1,2}|1[0-7][0-9]|180)(\.[0-9]{1,3})[o*°⁰˙˚•·º]?($|\b)")  # match degrees with decimals only

RE_DEGMIN = re.compile(r"^|\b([0-9]{1,2}|1[0-7][0-9]|180) |['`′/](00|15|30|45)[\"″'`′/]?($|\b)")     # coarse match of degrees + minutes

RE_MINSSECS = re.compile(r"^|\b([0-6][0-9])['`′/]( ?[0-6][0-9])?[\"″'`′/]?($|\b)")   # match minutes only (and optionally seconds)
...
# UTM coordinates
# pre-compiled regex patterns
RE_NORTHEAST = re.compile(r"^|\b([1-9]?[ ,.]?[0-9]{3}[ ,.]?[0-9]00) ?m?(N|North|E|East)?($|\b)")
...
LOC_TYPES_PASSLIST_NOM = ['municipality', 'city', 'borough', 'town', 'village', 'hamlet', 'locality']
...
class CoordinatesExtractor(Task):
    ...
    def _create_coordinate_result(self, input:CoordinateInput, lons, lats) -> TaskResult:
...
class GeoCoordinatesExtractor(CoordinatesExtractor):
    ...
    def _extract_coordinates(self, input:CoordinateInput):
        ...
        lon_pts, lat_pts = self._extract_lonlat(input, ocr_blocks, lon_minmax, lat_minmax)
    ...
    def _extract_lonlat(self, input:CoordinateInput, ocr_text_blocks, lon_minmax, lat_minmax):
        print('starting extraction')
        lon_clue = (lon_minmax[0] + lon_minmax[1])/2    # mid-points of lon/lat hint area
        lat_clue = (lat_minmax[0] + lat_minmax[1])/2    # (used below to determine if a extracted pt is lon or lat)  
            ...
            if not is_match:
                ...
                for m in matches_iter:
                    ...
                    if any(x for x in m_groups):
                        ...
                        is_match = True
                        m_span = (m.start(), m.end(), len(block['text']))
                        deg_matches[idx] = (m_groups, m_span)   # put in same dict as deg_matches
            if not is_match:
        ...
        for idx, (groups, span) in deg_matches.items():
            ...
            if idx+1 in minsec_matches and not is_decimal: 
                ...
                font_height = ocr_text_blocks[idx]['bounding_poly'].vertices[2].y - ocr_text_blocks[idx]['bounding_poly'].vertices[0].y
                blocks_y_separation = ocr_text_blocks[idx+1]['bounding_poly'].vertices[0].y - ocr_text_blocks[idx]['bounding_poly'].vertices[2].y
    ...
    def _check_degree_increments(self, minutes:int, seconds:int) -> bool:
        ...
        return (minutes * 60 + seconds) % 225 == 0 or (minutes * 60 + seconds) % 300 == 0
    ...
    def _validate_lonlat_extractions(self, input:CoordinateInput, lon_results, lat_results, im_size, roi_xy=[]):
        ...
        if num_lon_pts >=2 and num_lat_pts==1:
            ...
            lst = [(k[0], k[1] ,v.get_pixel_alignment()[1]) for k,v in lon_results.items()]
            ...
            if deg_range != 0 and pxl_range != 0:
                deg_per_pxl = abs(deg_range / pxl_range)    # TODO could use geodesic dist here?
            ...
            lst = [(k[0], k[1] ,v.get_pixel_alignment()[0]) for k,v in lat_results.items()]
            max_pt = max(lst, key=lambda p:p[1])
            min_pt = min(lst, key=lambda p:p[1])
            pxl_range = max_pt[1] - min_pt[1]
            deg_range = max_pt[0] - min_pt[0]
    ...
    def _remove_outlier_pts(self, input:CoordinateInput, deg_results, i_max, j_max):
        ...
        for idx, this_group in enumerate(deg_groups):
            if len(this_group) > 1:
                ...
                i_span = (max(i_vals) - min(i_vals)) * len(this_group)  # max pxl span (boosted by group size)
    ...
    def _filter_secondary_map(self, coord_result):
        ...
        min_coord, max_coord = min(coord_result, key = lambda x: x[1]), max(coord_result, key = lambda x: x[1])
        ...
        if min_coord != max_coord:
            ...
            print(f'delta pixel: {delta_pixel}\tdelta deg:{delta_deg}\tratio: {delta_pixel_deg}')
    ...
    def _get_geofence(self, geofence, dms_matches, ocr_text_blocks):
        degrees = []
        for idx, (_, _, deg_parsed) in dms_matches.items():
            # reduce parsed value to middle point
            bounding_box = ocr_text_blocks[idx]['bounding_poly']
            (x,y) = (bounding_box.vertices[0].x + bounding_box.vertices[2].x) / 2, (bounding_box.vertices[0].y + bounding_box.vertices[2].y) / 2
...
class UTMCoordinatesExtractor(CoordinatesExtractor):
    ...
    def _extract_utm(self, input:CoordinateInput, ocr_text_blocks, lonlat_results, lon_minmax, lat_minmax, lon_sign_factor=1.0):
        ...
        for idx, groups, span in ne_matches:
            ...
            if abs(utm_dist - northing_clue) < abs(utm_dist - easting_clue):
                # latitude keypoint (y-axis)
                if utm_dist >= northing_clue-northing_range and utm_dist <=  northing_clue+northing_range:
                    # valid latitude point
                    x_ranges = (0.0, 1.0) if span[2]==0 else (span[0]/float(span[2]), span[1]/float(span[2]))
                ...
                # longitude keypoint (x-axis)
                if utm_dist >= easting_clue-easting_range and utm_dist <=  easting_clue+easting_range:
                    # valid longitude point
                    x_ranges = (0.0, 1.0) if span[2]==0 else (span[0]/float(span[2]), span[1]/float(span[2]))
...
class GeocodeCoordinatesExtractor(CoordinatesExtractor):
    ...
    def _extract_coordinates(self, input:CoordinateInput):
        ...
        lon_pts, lat_pts = self._extract_places(ocr_blocks, (lon_pts, lat_pts), lon_minmax, lat_minmax, lon_sign_factor, roi_xy,
            use_google_maps=True, geocode_path=os.path.join(GEOCODE_CACHE, input.input.raster_id + '.pkl'))
        
        return lon_pts, lat_pts
    
    def _extract_places(self, ocr_text_blocks, lonlat_results, lon_minmax, lat_minmax, lon_sign_factor=1.0, roi_xy=[], use_google_maps=True, geocode_path=''):
        ...
        lon_range = (abs(lon_minmax[1] - lon_minmax[0])/2)*0.5  # use a smaller fov for geo-coding!
        ...
        if using_google:
            ...
            geocoder = Nominatim(timeout=timeout, user_agent="uncharted-cma-challenge-geocoder")
        ...
        for idx, block in enumerate(ocr_text_blocks):
            ...
            if roi_xy:
                x = self._get_center_x(block['bounding_poly'], (0.0, 1.0))
                y = self._get_center_y(block['bounding_poly'])
                if not self._in_bounds(x, roi_xy[0]) or not self._in_bounds(y, roi_xy[1]):
                    # this OCR block is not in ROI, so skip geocoding for this OCR block
                    continue

            viewbox = [Point(lat_minmax[1], lon_sign_factor*lon_minmax[1]), Point(lat_minmax[0], lon_sign_factor*lon_minmax[0])]
            
            places = geocode_cache.get(block['text'], None)
            if places is None:
                if using_google:
                    # run Google geo-coder 
                    places = geocoder.geocode(block['text'], exactly_one=False, timeout=timeout, region="us", bounds=viewbox)
                else:
                    # run Nominatim geo-coder
                    #featuretype='settlement' #'city,settlement'
                    places = geocoder.geocode(block['text'], exactly_one=False, limit=4, country_codes="us", viewbox=viewbox) #featuretype=featuretype)
                if places:
            ...
            for p_idx, place in enumerate(places):
                ...
                if this_lon >= lon_minmax[0] and this_lon <= lon_minmax[1] and this_lat >= lat_minmax[0] and this_lat <= lat_minmax[1]:
            ...
            if place_dists:
                ...
                x_pixel = self._get_center_x(ocr_text_blocks[idx]['bounding_poly'], (0.0, 1.0))
                y_pixel = self._get_center_y(ocr_text_blocks[idx]['bounding_poly'])
                lonlat_deg = (places[p_idx_match].longitude*lon_sign_factor, places[p_idx_match].latitude)
