...
class Tiler(Task):
    ...
    @staticmethod
    def process(image: MapImage, tile_size: List[int] = [1024, 1024], overlap: float = 0.00) -> List[MapTile]:
        ...
        for y in range(0, original_height, step_y):
            for x in range(0, original_width, step_x):
                ...
                if tile_array.shape[0] < tile_size[1] or tile_array.shape[1] < tile_size[0]:
                    padded_tile = np.zeros((tile_size[1], tile_size[0], 3), dtype=tile_array.dtype)

                    padded_tile[:height, :width] = tile_array
                    tile_array = padded_tile

                maptile = MapTile(x_offset=x,
                                  y_offset=y,
                                  width=tile_size[0],
                                  height=tile_size[1],
...
class Untiler(Task):
    ...
    def process(tiles: List[MapTile]) -> MapImage:
        ...
        assert all(i.predictions is not None for i in tiles), "Tiles must have predictions attached to them."
        all_predictions = []
        map_path = tiles[0].map_path
        for tile in tiles:
            for pred in tqdm(tile.predictions, desc="Reconstructing original map with predictions on tiles"):
                ...
                global_prediction = MapPointLabel(
                    ...
                    x1 = x1 + x_offset, # Add offset of tile to project onto original map.
                    y1 = y1 + y_offset,
                    x2 = x2 + x_offset,
                    y2 = y2 + y_offset,
                    score=score,
