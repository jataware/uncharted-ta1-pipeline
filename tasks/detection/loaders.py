...
class CompetitionCOCOLoader:
    ...
    BAD_JSON = ['AZ_PrescottNF_basemap.json']  # Manually filter files with .json labels that dont map to any raster map.
    ...
    def _fetch_map_paths(self, data_dir: str) -> List[str]:
        ...
        paths = [os.path.join(data_dir, file.replace('.json', '.tif')) for file in os.listdir(data_dir) if '.json' in file and file not in self.BAD_JSON]
        return paths
    ...
    def _fetch_bitmask_paths(data_dir: str, map_paths) -> Dict[str, List[str]]:
        ...
        out_dict = {}
        map_filenames = sorted([os.path.basename(p).split('.')[0] for p in map_paths], key=len, reverse=True) # Sort by length so the most specific match is used first. We'll use this logic to remove duplicate bitmask matches.
        all_files = os.listdir(data_dir)
        bitmask_paths = [os.path.join(data_dir, f) for f in all_files if f.split('.')[0] not in map_filenames]
    ...
    @staticmethod
    def _filter_bitmask_paths(bitmask_paths: List[str], label_names: List[str]) -> List[str]:
    ...
    def _fetch_map_metadata(map_path: str) -> Dict[str, Union[str, int]]:
        return {'id': filename_to_id(map_path),
    ...
    # not sure what to do about this.
        ...
        for i in range(1, num_features + 1): # Iterate through each feature in the bitmask. Each feature corresponds to a point equal to 1.
            component = (labeled == i)
            ...
            bounding_boxes.append([int(cmin), int(rmin), int(cmax - cmin), int(rmax - rmin)]) # x, y, width, height
    ...
    def process(self,
        ...
        for map_name, label_paths in tqdm(label_paths_dict.items(), desc='Processing bitmasks'):
            ...
            for bboxes, id, label_name in zip(all_bboxes, ids, detected_label_names):
                ...
                if category_id is not None:
                    ...
                    raise RuntimeError(f'Category id not found for label name: {label_name}')
        ...
        self.coco_dict = filter_coco_images(self.coco_dict) # Filter out images which have no annotations.
