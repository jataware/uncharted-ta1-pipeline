...
def filter_coco_images(coco_dict: Dict) -> Dict:
    ...
    print(f'Filtering {len(coco_dict["images"]) - len(filtered_coco_dict["images"])} out of {len(coco_dict["images"])} images without annotations')
...
def get_s3_creds():
    ...
    if url is None:
        raise RuntimeError('AWS S3 endpoint URL not found in environment variables.'
...
def ensure_local_cache_and_file(cache_dir, filename, bucket_name, s3_key):
    ...
    if not os.path.exists(local_file_path):
        ...
        last_modified_date = datetime.datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')
        print(f"File {filename} already exists in cache. Last modified on: {last_modified_date}")
