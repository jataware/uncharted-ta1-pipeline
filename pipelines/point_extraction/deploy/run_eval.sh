...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
    echo "Usage: $0 <input_dir> <output_dir> <hints_dir>"
    exit 1
fi
...
docker run \
    ...
    -v $3:/hints \
    --net lara \
    uncharted/lara-point-extract:dry-run \
         -m pipelines.point_extraction.run_pipeline \
        --input /input \
        --output /output \
        --legend_hints /hints \
        --bitmasks \
        --model_point_extractor pipelines/point_extraction_weights/lara_yolo_20240425_dryrun_best.pt \
